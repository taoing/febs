package com.taoing.quartz.service.impl;

import com.taoing.common.annotation.CronTag;
import com.taoing.common.service.impl.BaseService;
import com.taoing.quartz.dao.JobMapper;
import com.taoing.quartz.domain.Job;
import com.taoing.quartz.service.JobService;
import com.taoing.quartz.util.ScheduleUtils;
...
public class JobServiceImpl extends BaseService<Job> implements JobService {
    ...
    /**
     * @PostConstruct 注解, 在bean完成初始化后调用注解方法
     */
    @PostConstruct
    public void init() {
    ...
    public List<Job> getSysCronClazz(Job job) {
        Reflections reflections = new Reflections("com.taoing.quartz.task");
        ...
        for (Class cls : annotated) {
            ...
            for (Method method : methods) {
                ...
                String params = String.format("%s(%s)", methodName,
                        Arrays.stream(methodParameters).map(
                                item -> item.getType().getSimpleName() + " " + item.getName())
