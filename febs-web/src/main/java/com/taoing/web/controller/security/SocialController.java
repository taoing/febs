...
import com.taoing.security.social.SocialUserInfo;
...
public class SocialController extends BaseController {
    ...
    @GetMapping("/social")
    public String socialIndex(HttpServletRequest request, HttpServletResponse response,
    ...
    public String socialBindSuccess(HttpServletRequest request, HttpServletResponse response) {
        /* 判断当前社交账号是否已经和别的系统账户绑定了,
        是的话删除其和别的系统账户的绑定关系
         */
        MyUser currentUser = super.getCurrentUser();

        ServletWebRequest servletWebRequest = new ServletWebRequest(request, response);
        String providerId = (String) sessionStrategy.getAttribute(servletWebRequest,
                FebsConstant.SESSION_KEY_SOCIAL_OPENID);
