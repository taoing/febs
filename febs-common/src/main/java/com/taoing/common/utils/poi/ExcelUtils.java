package com.taoing.common.utils.poi;

import com.csvreader.CsvWriter;
import com.taoing.common.annotation.ExportConfig;
import com.taoing.common.utils.poi.convert.ExportConvert;
import com.taoing.common.utils.poi.pojo.ExportItem;
...
public class ExcelUtils {
    ...
    public boolean toExcel(List<?> data, String sheetName, ExportHandler handler, OutputStream out) {
        ...
        while (index <= (sheetNo == 0.0 ? sheetNo : sheetNo - 1)) {
            ...
            for (int i = 0; i < exportItems.size(); i++) {
                ...
                POIUtils.setColumnWidth(sheet, i, exportItems.get(i).getWidth(),
                        exportItems.get(i).getDisplay());
    ...
    public boolean toCsv(List<?> data, String path) {
        try {
            ...
            String[] csvHeadersArr = exportItems.stream().map(ExportItem::getDisplay)
                    .toArray(String[]::new);
    ...
    private String convertCellValue(Object oldValue, String format) {
        try {
            String protocal = format.split(":")[0];

            // 键值对字符串解析: s:1=男,2=女
            if ("s".equalsIgnoreCase(protocal)) {
            ...
            if ("c".equalsIgnoreCase(protocal)) {
    ...
    private void requiredbuilderParams() {
        if (mClass == null) {
            throw new IllegalArgumentException("请先使用" +
                    "com.taoing.common.utils.poi.ExcelUtils.builder(Class<?>)构造器初始化参数");
        }
    }
    ...
    private void requiredexportParams() {
        if (mClass == null || mResponse == null) {
            throw new IllegalArgumentException(
                    "请先使用com.taoing.common.utils.poi.ExcelUtils.export(Class<?>, " +
                            "HttpServletResponse)构造器初始化参数"
            );
        }
    }
}
